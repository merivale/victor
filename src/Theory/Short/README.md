...
(I am here treating `"the sun"` as a proper name, because it is only in the fourth layer of my model that I am able to generate it as a definite description; see [the README in the `src/Theory/Object` directory](https://github.com/merivale/victor/tree/master/src/Theory/Object). I will do this sort of thing several times in what follows, and I will not bother with this proviso again.)
...
My model is not yet able to generate sentences like these, however, and rigidly insists on placing the outputs of all time arguments at the end of the sentence. As noted at the end of [section 3 of the main README](https://github.com/merivale/victor#3-ambiguity), my model currently implements only informational choices, leaving stylistic choices for a later update.
