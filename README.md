...
Pronouns and proper names are generated by objects, which we find already in the nuclues of English messages. To generate nouns and noun phrases - like `"water"`, `"some water"`, `"any water"`, `"that water over there"` - we require some elaboration. I account for the data here with three elaborations, and quite a few new variable types:
...
It isn't as complicated as it looks. The *haystack*, first of all, consists predominantly of a *category*, which is encoded in a noun: `"car"`, `"frog"`, `"turnip"`, `"water"`, and so on. This is yet another unearthed variable type; users must encode their categories for themselves, inputting the corresponding noun directly into the system. Categories can optionally be flanked by *properties* and *restrictions*, the former encoded in adjectives (e.g. `"fresh"`, `"controversial"`) and the latter encoded in more complex phrases (e.g. `"from Spain"`, `"in the room"`); thus `"fresh fruit from Spain"`, `"controversial philosopher in the room"`, etc. Both of these, again, are unearthed variable types. We have already met properties in the context of counters. In fact, my model almost has the means to unearth descriptions: they are essentially just balances, like those we have already seen in the nucleus. But there is a complication that makes the implementation of this non-trivial, and consequently best left for a later update; I will explain this complication later.

An *indirection*, next, consists of a *pointer*, a boolean flag, and a haystack. Pointers are responsible for the articles `"the"`, **`"this"`**, and **`"that"`**, and the relative forms of the pronouns (`"my"`, `"his"`, `"your"`, etc.). These last are triggered by the `RelatedTo Object` value, where the object variable, as you would expect, is responsible for the choice of pronoun. Indirections, then, fetch up in noun phrases like `"the road to nowhere"`, `"these shoes"`, `"my favourite film"`, and `"your best interests"`. The boolean flag triggers the insertion of the word `"other"`, as in, `"my *other* favourite film"`.

(A better model would allow the possibility of indirections without a haystack (or the boolean flag), permissible with all pointers except `The`. The result of these would be the unadorned determiners `"this"`, `"these"`, `"that"`, and `"those"`, as the *second* relative form of the pronouns (`"mine"`, `"yours"`, `"theirs"`, etc.). This is not a difficult addition, but as always, one has to stop somewhere. I will make this modification in a later update.)

And now we have all we need to carve up `INDIRECT` messages, which comprise - alongside their input message - an indirection and a specification of the object in the nucleus that they are targetting. Balancing objects are indexed from 0, and so I use -1 to pick out the main object. For example:

```elm
INDIRECT -1 ( The, "sun" ) ( Other, ( Be, "hot" ) )
    -> "The sun is hot."

INDIRECT 0 ( RelatedTo Hearer, "style" ) ( Speaker, ( Do "like", [ Other ] ) )
    -> "I like your style."
```

The point of the `INDIRECT` elaboration is to secure reference to an object *indirectly*, by "pointing" to an element or some elements within the given haystack. There is plenty more that could be said here (the semantics of `"the"` is a hotly contested philosophical topic). But discussion of it here would take us much too far from our main concerns.

Pointers are encoded in articles; *quantifiers*, on my account, are encoded in determiners (`"all"`, `"any"`, `"each"`, `"every"`, `"some"`, `"most"`, etc.). Structurally, the `ENUMERATED` and `AMASSED` elaborations operate in exactly the same way as the `INDIRECT` elaboration, except they they have a quantifier instead of a pointer. 
